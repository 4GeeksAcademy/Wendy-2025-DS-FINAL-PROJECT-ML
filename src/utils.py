...
# To perform Scaling of characteristics:
def scale_datasets(training_data, testing_data, anonymous_data):
    scaler = MinMaxScaler()
    training_scaled = pd.DataFrame(scaler.fit_transform(training_data), columns=training_data.columns, index=training_data.index)
    testing_scaled = pd.DataFrame(scaler.transform(testing_data), columns=testing_data.columns, index=testing_data.index)
    anonymous_scaled = pd.DataFrame(scaler.transform(anonymous_data), columns=anonymous_data.columns, index=anonymous_data.index)
    return training_scaled, testing_scaled, anonymous_scaled, scaler

# To calculate training and testing metrics:
def calculate_metrics(y_train, y_pred_train, y_test, y_pred_test):
    ...
    diff_mape = abs(mape_train - mape_test)
    diff_r2 = abs(r2_train - r2_test)   
    metrics_df = pd.DataFrame({
        "MAE": [mae_train, mae_test, diff_mae],
        "MAPE": [mape_train, mape_test, diff_mape],
        "R2": [r2_train, r2_test, diff_r2]      
